// FlowOS Reach — Instagram posting via Zernio
...
//
// Actions:
//   resolve_accounts → resolve_authors via Zernio (returns { authors: [{urn,name,kind}] })
//   publish_now      → publish_now via Zernio (imageUrl is required for IG)
...
export default async function handler(req) {
  ...
  if (action === "resolve_accounts") {
    return forwardToZernio(req, { ...body, action: "resolve_authors", platform: "instagram" });
  }
  if (action === "publish_now") {
    return forwardToZernio(req, { ...body, platform: "instagram" });
  }

  return errResponse(`Unknown action: ${action}. Supported: resolve_accounts, publish_now`);
}
