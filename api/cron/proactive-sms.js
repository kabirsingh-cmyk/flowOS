 ...
 * POSTs to /api/proactive-sms which classifies analytics signals into SMS rules
 * (S1..S4), generates drafts via Claude, persists to proactive_sms (status
 * 'proactive_draft'). SmsCenter surfaces them for human review.
 */
...
export default async function handler(req) {
  ...
  for (const tenantId of tenantIds) {
    ...
    try {
      const res = await fetch(`${origin}/api/proactive-sms`, {
        method:  "POST",
        headers: {
  ...
  console.log(`[cron/proactive-sms] ${passed} ok · ${failed} failed`, results);
