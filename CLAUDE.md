...
**Dropped from catalog 2026-05-18**: VWO, AB Tasty, Loops.so (scope cut — Optimizely covers A/B Testing alone). MailerLite, Moosend, ActiveCampaign, Hunter (scope cut — Klaviyo + Mailchimp + SendGrid cover email; lifecycle email use case is satisfied). Attentive (scope cut — Klaviyo SMS is sufficient for now; Twilio retained for transactional/dev-side messaging only). To restore any of these: row in `seed.jsx connectorCatalog` + default `connectorState` + `brandConnectorStates.erickson`, entry in `agents.jsx CONNECTOR_LABELS`, id in `api/brand-import.js CONNECTOR_IDS`, slug in `api/composio.js` or `api/pipedream.js` APP_MAP (+ matching verify script), and (for direct ones) `/api/<id>.js` + a row in `DIRECT_API_ROUTES` (+ `DIRECT_OAUTH_ROUTES` for OAuth + `/api/<id>-auth.js`).
...
| `GET  /api/msads-auth` | Direct OAuth surface. Single GET endpoint with three modes: `?init=1&tenantId=…` returns `{ok, authorizeUrl}`; `?code=…&state=…` is the provider's callback (exchanges code, persists tokens, 302s to `/oauth-callback.html?direct_connected=msads`); `?status=1&tenantId=…` returns `{connected, expiresAt}` for the frontend polling loop. State is HMAC-signed. Tokens persisted with `secret_kind = "oauth_tokens"`. |
| `POST /api/msads` | Post-connection actions for the msads Direct OAuth connector. Actions: `disconnect` (wipes tokens + flips channels), `refresh` (forces a refresh-token exchange — usually unnecessary since `loadOAuthCredential` auto-refreshes at-read). |
