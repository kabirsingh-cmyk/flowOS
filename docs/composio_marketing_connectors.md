...
- **39 of 40** connectors wired end-to-end (23 Composio + 5 Pipedream + 6 Direct: Replicate, Higgsfield, Luma, Optimizely, AudioStack, WordPress).
...
- **1 Direct API connector skipped**: Spotify Ads. Spotify Ad Studio has no documented public OAuth flow — the API is partner-gated through `partners@spotify.com`. Tile stays in the catalog as an important channel; OAuth scaffolding in `api/lib/directCredentials.js` + `app/workspaces4.jsx` + `oauth-callback.html` is retained as latent infrastructure so wiring becomes a one-route add when/if Spotify opens up.
